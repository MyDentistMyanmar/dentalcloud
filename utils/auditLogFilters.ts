...
export type AuditLogFilterRow =
  ...
  | {
      kind: 'reschedule';
      sortDate: string;
      rescheduleLog: Pick<AppointmentRescheduleLog, 'created_at'> & Partial<Pick<AppointmentRescheduleLog, 'original_date' | 'new_date' | 'appointment_date'>>;
    }
...
export const getAuditLogEventDate = (row: AuditLogFilterRow): string => {
  ...
  if (row.kind === 'reschedule') {
    return (
      row.rescheduleLog.created_at?.slice(0, 10) ||
      row.rescheduleLog.appointment_date ||
      row.rescheduleLog.new_date ||
      row.rescheduleLog.original_date ||
      ''
    );
  }
